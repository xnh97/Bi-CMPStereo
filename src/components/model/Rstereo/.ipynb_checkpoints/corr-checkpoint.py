...
class AGCL:
    def __init__(self, fmap1, fmap2, att=None, pos_encoding=None, self_att=None):
        self.fmap1 = fmap1
        self.fmap2 = fmap2
        self.att = att
    ...
    def __call__(self, flow, extra_offset, small_patch=False, iter_mode=False, fusion=False):
        if iter_mode:
            corr = self.corr_iter(self.fmap1, self.fmap2, flow, small_patch)
        else:
            corr = self.corr_att_offset(self.fmap1, self.fmap2, flow, extra_offset, small_patch, fusion=fusion)
        return corr
    ...
    def corr_att_offset(self, left_feature, right_feature, flow, extra_offset, small_patch, fusion=False):
        N, C, H, W = left_feature.shape
        
        # 对双目输入特征进行 cross 注意力处理，仅 1/16 特征图
        if self.att is not None:
            # 左目 1/16 特征图添加位置编码
            x_tmp = self.pos_encoding(left_feature)
            # 2 维特征图平铺为 1 维 [n, c, h, w]——>[n, h*w, c]
            left_feature = x_tmp.permute(0, 2, 3, 1).reshape(x_tmp.shape[0], x_tmp.shape[2] * x_tmp.shape[3], x_tmp.shape[1])
            # 右目 1/16 特征图添加位置编码，平铺 [n, c, h, w]——>[n, h*w, c]
            x_tmp = self.pos_encoding(right_feature)
            right_feature = x_tmp.permute(0, 2, 3, 1).reshape(x_tmp.shape[0], x_tmp.shape[2] * x_tmp.shape[3], x_tmp.shape[1])

            # self 注意力机制分别处理双目 1/16 特征图
            left_feature, right_feature = self.self_att(left_feature, right_feature)
        ...
        for i in range(len(lefts)):
            ...
            if fusion:
                corrs.append(corr.unsqueeze(dim=1))
            else:
                corrs.append(corr)
