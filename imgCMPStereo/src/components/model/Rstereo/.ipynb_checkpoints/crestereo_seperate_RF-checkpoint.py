...
from .update import BasicUpdateBlock, BasicMotionEncoder
from .attention.position_encoding import PositionEncodingSine
from .attention.transformer import LocalFeatureTransformer
from .attention.cross_channel import EventImage_ChannelAttentionTransformerBlock
from .fusion.RFNet_v1 import RFNet
...
class CREStereo(nn.Module):
    def __init__(self, base_channels=256, max_disp=192, mixed_precision=False, test_mode=False, iters=10, cross_modality=False):
        ...
        # self.recurrent_fusion1 = RFNet(in_channels=base_channels, out_channels=base_channels, flow_channels=2, 
        #                               base_1_channels=256, base_2_channels=64, stage=3)
        self.fusion_corr =  nn.Sequential(nn.Conv3d(8, 64, kernel_size=1, stride=1, padding=0),
                                                                            nn.LeakyReLU(negative_slope=0.1, inplace=True),
                                                                            nn.InstanceNorm3d(64, affine=True),
                                                                            nn.Conv3d(64, 4, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1)))
    ...
    def forward(self, image1, image2, image_V):
        ...
        corr_fn_dw16 = AGCL(fmap1_dw16, fmap2_dw16, att=self.cross_att_fn, 
                                                                        pos_encoding=pos_encoding_fn_small, self_att=self.self_att_fn)
        ...
        for itr in range(self.iters//2):  # 循环 10//2=5 次
            ...
            if itr==1:
                x = self.recurrent_fusion(ini_state=fmap1_dw16, up_left=fmap1_dw8, up_right=fmap2_dw8, 
                                                           flow=flow_dw16.detach())
                corr_fn_dw16_ = AGCL(x, fmap2_dw16)
            ...
            if it==0:
                ...
                out_corrs_ori = corr_fn_dw16(flow=flow_dw16, 
                                                              extra_offset=offset_dw16,
                                                              small_patch=small_patch,
                                                              fusion=True)
                out_corrs_upt = corr_fn_dw16_(flow=flow_dw16, 
                                                              extra_offset=offset_dw16,
                                                              small_patch=small_patch,
                                                              fusion=True)
                out_corrs = self.fusion_corr(torch.cat([out_corrs_ori, out_corrs_upt], dim=1))
                b, _, c, h, w = out_corrs.shape
                out_corrs = out_corrs.reshape(b, -1, h, w)
            ...
            #     net_dw16, up_mask, delta_flow = self.update_block(net_dw16, inp_dw16, out_corrs, flow_dw16)   
            # else:
            #     net_dw16, up_mask, delta_flow = self.update_block1(net_dw16, inp_dw16, out_corrs, flow_dw16)   
            ...
            flow_up = -4*F.interpolate(flow,
              ...
              #  x = self.recurrent_fusion(ini_state=fmap1_dw16, up_left=fmap1_dw8, up_right=fmap2_dw8, 
                #                                           flow=flow_dw16.detach())
                #fmap1_dw16_ = fmap1_dw16 + x
                #corr_fn_dw16_ = AGCL(fmap1_dw16_, fmap2_dw16)
                # corr_fn_dw16.resetLeft(fmap1_dw16_)
        ...
        for itr in range(self.iters//2):
            ...
            if itr==1:
                x = self.recurrent_fusion(ini_state=fmap1_dw8, up_left=fmap1, up_right=fmap2, 
                                                           flow=flow_dw8.detach())
                corr_fn_dw8_ = AGCL(x, fmap2_dw8)
            ...
            if itr==0:
                ...
                out_corrs_ori = corr_fn_dw8(flow=flow_dw8, 
                                                      extra_offset=offset_dw8, 
                                                      small_patch=small_patch,
                                                      fusion=True)
                out_corrs_upt = corr_fn_dw8_(flow=flow_dw8, 
                                                      extra_offset=offset_dw8, 
                                                      small_patch=small_patch,
                                                      fusion=True)
                out_corrs = self.fusion_corr(torch.cat([out_corrs_ori, out_corrs_upt], dim=1))
            ...
            flow_up = -2*F.interpolate(flow,
              ...
              #  x = self.recurrent_fusion(ini_state=fmap1_dw8, up_left=fmap1, up_right=fmap2,
                #                                           flow=flow_dw8.detach())
               # fmap1_dw8_ = fmap1_dw8 + x
                # corr_fn_dw8.resetLeft(fmap1_dw8_)
                #corr_fn_dw8_ = AGCL(fmap1_dw8_, fmap2_dw8)
