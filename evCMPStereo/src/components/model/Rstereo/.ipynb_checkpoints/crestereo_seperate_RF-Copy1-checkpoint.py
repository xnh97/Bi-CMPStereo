...
class CREStereo(nn.Module):
    def __init__(self, base_channels=256, max_disp=192, mixed_precision=False, test_mode=False, iters=10, cross_modality=False):
        ...
        #self.update_block2 = BasicUpdateBlock(hidden_dim=self.hidden_dim, cor_planes=4*9, mask_size=4)
    ...
    def forward(self, image1, image2, image_V):
        ...
        for itr in range(self.iters//2):  # 循环 10//2=5 次
            ...
            if itr == 1:
                # fmap1_dw16 = self.RF(net=fmap1_dw16, left=fmap1_dw16, right=fmap2_dw16, flow=flow_dw16.detach())
                # corr_fn_dw16.resetLeft(fmap1_dw16)
                x, _ = self.recurrent_fusion(last_state=None, up_left=fmap1_dw8, up_right=fmap2_dw8, 
                                                           flow=flow_dw16.detach(), init=True, ini_state=fmap1_dw16)
                fmap1_dw16_ = fmap1_dw16 + x
                #fmap1_dw16_ = adaptive_instance_normalization(fmap1_dw16_, fmap1_dw16)
        ...
        for itr in range(self.iters//2):
            ...
            if itr<2:
                ...
                net_dw8, up_mask, delta_flow = self.update_block1(net_dw8, inp_dw8, out_corrs, flow_dw8)   
            ...
            if itr == 1:
                x, _ = self.recurrent_fusion(last_state=None, up_left=fmap1, up_right=fmap2,
                                                           flow=flow_dw8.detach(), init=True, ini_state=fmap1_dw8)
                fmap1_dw8_ = fmap1_dw8 + x
                # fmap1_dw8_ = adaptive_instance_normalization(fmap1_dw8_, fmap1_dw8)
